...
def fix_password_hash():
    ...
    try:
        with connection.cursor() as cursor:
            ...
            # Verify superuser
            cursor.execute(
                "SELECT COUNT(*) FROM dating_user WHERE is_superuser = true;"
            )
            ...
            cursor.execute(
                "SELECT email, is_superuser, is_staff, is_active FROM dating_user WHERE email = %s;",
                (email,),
            )
            user = cursor.fetchone()
            if user:
                print(
                    f"✅ User verified: {user[0]} (superuser: {user[1]}, staff: {user[2]}, active: {user[3]})"
                )
            else:
            ...
            print(
                f"   URL: https://bondah-backend-api-production.up.railway.app/admin/"
            )
            print(f"   Email: {email}")
